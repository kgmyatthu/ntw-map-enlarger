...
/** Sink the heightmap bias with the enlargement: b -> b - |b|*(f-1), floored at -10
 * (a bias already below -10 is left untouched, never raised).
 * -5 at x1.5 -> -7.5; -8 at x2 -> -10 (clamped); +5 at x1.5 -> 2.5; no bias attribute = no-op. */
export function shiftBias(xml: string, factor: number): string {
  return xml.replace(/(\bbias=')([0-9.\-]+)(')/g, (m: string, a: string, v: string, c: string) => {
    ...
    return `${a}${Math.max(Math.min(b, -10), b - Math.abs(b) * (factor - 1)).toFixed(6)}${c}`;
  });
}
